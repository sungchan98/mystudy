...
public class CustomerListServlet extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    out.println("<a href='/member/form.html'>새 고객</a>");

    try {
      out.println("<table border='1'>");
      out.println("    <thead>");
      out.println("    <tr> <th>번호</th> <th>이름</th> <th>아이디</th> <th>가입일</th> </tr>");
      out.println("    </thead>");
      out.println("    <tbody>");
      ...
      for (Customer customer : list) {
        out.printf(
            "<tr> <td>%d</td> <td><a href='/customer/view?no=%1$d'>%s</a></td> <td>%s</td> <td>%s</td> </tr>\n",
            customer.getNo(),
            customer.getName(),
            customer.getId(),
